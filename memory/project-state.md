...
## Latest: HOTSPOT TIP on the "failed to find lobby" screen (#224, 2026-07-21, VRmike, branch build/224-hotspot-tip-on-the). A player who follows a join link and can't connect (the menu showing a connection error — the common cause being everyone on mobile data, where carrier NAT blocks P2P) now sees a help box explaining the hotspot workaround. **UI-ONLY — no networking/game-logic change.** FULL DETAIL: `notes/connection-ui.md`.

- **WHERE THE FAILURE SCREEN LIVES.** There is no separate "failed to find lobby" screen — a failed guest join surfaces as the **menu screen (`#menu`) with an error string in `#menuError`**. `js/net.js _startGuest` emits `onStatus('error', msg)` for every join failure ("Room X not found", the 10 s connect timeout, PeerJS load fail, etc.); `js/main.js handleStatus('error')` routes it to `ui.menuError(detail, true)` when the menu is visible (in-game → the feed instead).
- **THE TIP.** New `#hotspotTip` box in the menu card (`index.html`), directly under `#menuError`, ABOVE the existing `.hint`/Create/Join controls (never crowds them). Two parts kept verbatim-ish per spec: (1) how-to — "On mobile data? … one player turns on their phone's hotspot, everyone else joins that wifi, the hotspot player hosts — you'll all connect directly over the local network."; (2) smaller muted note — the client-isolation fallback ("try a different phone's hotspot").
- **ONLY ON FAILURE.** `ui.menuError(msg, showTip=false)` now toggles the tip: it ships `.hidden` and is unhidden ONLY when `showTip` is true, which ONLY the connect-error path passes. "Connecting…", cleared errors, "enter a room code" validation, and the `backToMenu` disconnect messages all leave it hidden — nobody sees troubleshooting advice before a real failure.
- **PHONE-FIRST STYLING (`css/style.css`).** Compact `.hotspot-tip` box (capped by the 320 px card, dark inset bg, accent2 heading), `.hotspot-tip-note` smaller + muted. Plus `#menu { overflow-y:auto }` + `#menu > .menu-card { margin-block:auto }` so a short phone can scroll to keep Create/Join reachable once the tip is shown (the flex-centring-clips-the-top fix; scoped to `#menu`, never the game canvas).
- **Files:** EDITED `index.html` (`#hotspotTip` markup), `css/style.css` (`.hotspot-tip*` + `#menu` scroll-safety), `js/ui.js` (`hotspotTip` el + `menuError(msg, showTip)` toggle), `js/main.js` (`handleStatus('error')` passes `true`), NEW `tools/check-hotspot-tip.mjs` (static guard: tip markup carries BOTH wording parts, defaults hidden, shows on the error path but NOT the connecting path), `notes/connection-ui.md`, this file.
- **VERIFY:** `node tools/check-hotspot-tip.mjs` ALL GREEN. `browser_check` (phone) boots clean (0 console errors); normal menu unchanged; loading with a `#ZZZZ` hash auto-joins a nonexistent room → real failure state renders the error + the readable tip, Create/Join untouched above (screenshot confirmed).

## Latest: LATE-JOINER SEES PROPS AS RED CUBES (#221, 2026-07-20, VRmike, playtest bug, screenshots posted, branch build/221-late-joiner-sees-props). A player joining a match in progress saw every disguised PROP rendered as a plain reddish box (e.g. `diner_chair`'s `#c0392b` primitive) instead of the furniture GLB — at the CORRECT position, and it never self-corrected. FULL DETAIL: `notes/netcode.md` (Late-joiner disguise swap-in section).

- **DIAGNOSIS (the report's mechanism guess was wrong — verified before fixing).** VRmike guessed the disguise identity was "only broadcast at transform time" so a late joiner never got it. Tracing all three named files disproved that: the per-tick **SNAPSHOT already carries every prop's `disguise` (+ `tool`, `alive`, `health`)** and it correctly rides the anti-cheat gate (`blindHunterSnapshot` withholds props from a blindfolded HIDING hunter; `hunterSafeSnapshot` keeps the disguise SHAPE for HUNTING hunters while blanking the roster NAME). So the wire was never the gap — **no `protocol.js`/`referee.js` change was needed.** The real ROOT CAUSE is client-side (`js/scene.js`): a disguise avatar's appearance `kind` was the fixed string `d:<type>`, which did NOT encode whether the disguise GLB had loaded. A late joiner whose GLBs were still downloading when the first snapshot arrived built the reddish **primitive placeholder** — and because the kind never changed, `syncPlayers` never rebuilt it once the GLB finished loading → **stuck red box forever**. Veterans dodged it only because their GLBs had long since loaded before anyone disguised.
- **THE FIX (`js/scene.js`, client render only).** New `_playerKind(p, animated)` — the ONE appearance-signature builder both `syncPlayers` (remote) and `_syncSelf` (local) route through — folds GLB readiness INTO the disguise kind: `d:<type>:glb` once the model is loaded, else `d:<type>:prim`. So the moment the GLB lands, the next snapshot's kind changes and the avatar rebuilds from placeholder → real furniture (exactly the pattern hunters already had via `hunter:cap`→`hunter:swat`). Backed by `_disguiseModelReady(type)` (mirrors the exact check `_buildPlayerMesh` uses, so kind + render can't disagree). PLUS `_ensureDisguiseModel(path)` — an on-demand single-GLB loader kicked from `_buildPlayerMesh` when a disguise references a model this client never queued via `_modelSlots` (deduped by `_disguiseLoading`, token-guarded), so the placeholder is only ever TEMPORARY, never permanent.
- **SIBLINGS SWEPT (plan step 3).** Held items (`tool`) + `alive` were checked for the same "announced once, never repeated" pattern — they're fine: both are re-applied every snapshot (`_applyHeldTool`, `mesh.visible = alive`), so they never stick like disguise did. Confirmed for late joiners by the new check.
- **Files:** EDITED `js/scene.js` (`_playerKind`/`_disguiseModelReady`/`_ensureDisguiseModel` + `_disguiseLoading` set; `syncPlayers` + `_syncSelf` use the helper; `_buildPlayerMesh` on-demand load; `buildWorld` clears the load-guard), NEW `tools/check-late-join-disguise.mjs` (drives the REAL `addPlayer`→`admitMidGame` join flow: hunter/prop late joiners in HUNTING get the disguise shape (+ tool/alive), a HIDING hunter joiner gets ZERO prop data, filter sanity, and static guards on the client swap), `notes/netcode.md`, `architecture.md`, this file.
- **VERIFY:** `node tools/check-late-join-disguise.mjs` ALL GREEN. Neighbour sweep GREEN: `check-flicker`, `check-blindfold`, `check-spectator`, `check-team-flip`. `browser_check` boots clean (0 console errors, menu renders). **OWED — VRmike in-game (headless can't render a GLB swap):** join a match already in progress → props look like furniture, not red cubes, within a beat of joining (a brief placeholder flash while the GLB downloads is fine; it must resolve, never stick).

## Latest: PAUSE MENU QoL PACK (#218, 2026-07-20, VRmike, screenshot posted, branch build/218-pause-menu-qol-pack). Five related pause-menu/UI fixes in one pass. FULL DETAIL: `notes/pause-menu.md` (QoL-pack section) + `notes/lobby-rename.md` (mid-game section).

- **1 · MID-GAME NAME CHANGE.** Pause scoreboard: your OWN row is highlighted (`.ps-self-row`) + click-to-edit (✎). Click → inline `<input.ps-name-edit>` (`ui._beginNameEdit`), **Enter/blur commits, Esc cancels**, works on touch. Commit relays the SAME `ui.onRename`→`C2S.RENAME` the lobby uses. **SURVIVES the reused-row ~15 Hz refresh** (the #213/7954df9 hazard VRmike warned of): `updatePauseScoreboard` skips rewriting the name cell while `this._psEditId === p.id`; the rest of the row keeps updating live. `hidePause()` blurs the box first (commits) then clears the marker.
- **2 · Netcode: rename now mid-match too.** `referee.applyRename` dropped its LOBBY-only early-return. Lobby → `broadcastLobby()`; mid-match → the new name just rides the next `broadcastSnapshot`, whose hunter variant (`hunterSafeSnapshot`) BLANKS a disguised prop's name → a mid-game rename can't leak a hiding prop to hunters. NO `broadcastLog` (would bypass the blank). Host validation identical (trim/cap-16/reject-empty/de-dupe).
- **3 · CLICK-OUTSIDE CLOSES.** `.pause-menu` IS the dim backdrop; a click with `e.target === pauseMenu` → `ui.onPauseOutsideClick` → `closePause(true)` — the EXACT Resume path (pointer lock requested synchronously in that real click).
- **4 · HALVED dim + blur.** `.pause-menu` bg `#060010cc→#06001066` (~0.80→0.40) and `backdrop-filter blur(3px)→blur(1.5px)`.
- **5 · POINTER-LOCK NAG de-scared.** Chrome refuses re-lock for ~1.25 s after an Esc-exit, and the menu opens via that Esc-exit — so a quick Resume/click-outside close often lands in the cooldown (NOT a real failure). `input.js pointerlockerror`→`onLockError()` (no message); `main.js` shows a small CALM hint ("Click to recapture the mouse") via `setClickToPlay(_, _, calm=true)`→`.overlay.overlay-hint` (no dim, small type, still click-anywhere-to-relock). Old scary red nag gone. Touch has no pointer lock → never fires there.
- **6 · VOTE-KICK BANNER OVERLAP.** Was fixed `top:62px`, overlapping the props' taunt button (88px) + red stop button (140px) on PC. Now `.votekick { top: calc(196px + env(safe-area-inset-top,0)) }` — below the whole top strip (clears taunt-stop 140+46=186) at any width; old 108px narrow override removed.
- **Files:** EDITED `shared/referee.js` (`applyRename` phase-gate lift), `shared/protocol.js` (RENAME doc), `css/style.css` (pause dim/blur halved, `.votekick` reposition + narrow-override removal, `.ps-self-row`/`.ps-editable`/`.ps-name-edit`, `.overlay.overlay-hint`), `js/ui.js` (`_beginNameEdit`, scoreboard self-row highlight + edit-survives-refresh guard, `setClickToPlay(calm)`, click-outside wiring, `hidePause` commit-on-close, edit state), `js/main.js` (`onPauseOutsideClick`, calm `onLockError`), `js/input.js` (pointerlockerror → messageless `onLockError`), `tools/check-lobby-rename.mjs` (section D rewritten: mid-game applied + snapshot-ride + hunter-blank), `tools/check-votekick.mjs` (NEW **M** banner-below-top-UI numeric, **N** rename-survives-refresh static), `notes/pause-menu.md`, `notes/lobby-rename.md`, this file.
- **VERIFY:** `node tools/check-lobby-rename.mjs` ALL GREEN (incl. new mid-game D). `node tools/check-votekick.mjs` ALL GREEN (A–N; M banner=196px clears taunt-stop 186px, N edit-state guards the name cell). `browser_check` boots clean (0 console errors, menu renders). **OWED — VRmike in-game (headless can't drive a 2-peer paused DOM):** rename mid-match propagates to a 2nd client (and stays blanked for a hunter on a disguised prop); click-outside resumes; a normal resume shows no scary nag; banner clear of the taunt button on PC.

## Latest: STOP TAUNT AUDIO ON PROP DEATH (#216, 2026-07-20, VRmike, playtest bug, branch build/216-stop-taunt-audio-on). A killed prop's taunt clip kept ringing out until it finished; it now cuts the INSTANT they die, on every screen in earshot (incl. the victim's own). Pure client-side audio bookkeeping riding the host's EXISTING authoritative death event — NO server/referee change. FULL DETAIL: `notes/audio-taunts.md` ("Cut taunt audio on prop death" section).

- **THE FIX.** `scene.stopTaunt(playerId)` already existed (immediate stop + dispose of that player's per-player emitter, safe no-op if silent). This build only WIRES it to the death/silence seams. Death signal is the referee's existing `eliminated{victim}` broadcast (fired by `_damagePlayer` on any death, reaches ALL clients).
- **SEAMS.** (1) Main death hook — `js/main.js onEvent case 'eliminated'` → `scene.stopTaunt(msg.victim)`; because the event is broadcast to everyone, this cuts the taunt on every phone/PC AND covers self-death (victim gets their own `eliminated`). (2) Self-death belt-and-suspenders — `onSnapshot` alive→dead flip also `scene.stopTaunt(state.selfId)` (the snapshot flip is a distinct path that can arrive first/alone; idempotent with the event). (3) Round end — `case 'roundOver'` → `scene.clearAllTaunts()`. (4) Disconnect — `js/scene.js syncPlayers` removal loop `stopTaunt(id)` before removing the mesh. (Return-to-lobby already cleared via `backToMenu`; preserved.)
- **Files:** EDITED `js/main.js` (eliminated hook, self-death flip, roundOver clear), `js/scene.js` (syncPlayers disconnect stop), NEW `tools/check-taunt-death-silence.mjs` (drives the REAL referee for the death event + models the taunt→track→death→stop lifecycle incl. self-death + source-asserts all four seams), `notes/audio-taunts.md`, this file.
- **VERIFY:** `node tools/check-taunt-death-silence.mjs` ALL GREEN (eliminated{victim} reaches every client; dead prop's taunt stopped+disposed on remote AND victim's own client; bystander taunt untouched; silent-stop no-op; round-end clears; all four source seams wired). `node tools/check-taunts.mjs` still ALL GREEN (no regression). **OWED — VRmike in-game:** kill a taunting prop mid-track both as another player AND as the dying prop; confirm instant silence on both screens.

## Latest: HELD-ITEM ALIGNMENT TUNER in the debug menu (#215, 2026-07-20, VRmike, branch build/215-held-item-alignment-tuner). Attempt 4 at lining up the rifle/grenade/finder in the hunter's hand + on the character model — automated offset guessing (#190/#208/#212) kept missing, so per VRmike this switches to a HUMAN-IN-THE-LOOP tuner. Builds the tuner + export ONLY; ships NO default-offset change. FULL DETAIL: `notes/held-item-tuner.md`.

- **THE ROOT INSIGHT (why 3 blind guesses missed).** The held item is placed by TWO independent code paths reading DIFFERENT numbers: the first-person viewmodel (`js/scene.js _buildViewModel`, HARDCODED transforms) and the third-person character model (`_buildHunterModel`, `character-models.json weapon.*` offsets). That split is the textbook "looks right in my hand, floats wrong in everyone else's view" signature (the reported bug lives in OTHER players' third-person views). Not unified in this build (own refactor, own risk) — the override is wired into BOTH sites instead.
- **WHAT SHIPS.** A `?debug=1`-gated "Held-item alignment" panel in the existing debug menu: per-item tabs (rifle/finder/grenade, ● marks the equipped one), seven +/− steppers with live values (pos X/Y/Z m, rot pitch/yaw/roll°, scale ×), per-item Reset, and Export (copy-pasteable JSON block → clipboard + selectable box). The override is a LAYER on the shipped defaults (zeros = no change), applied live at both mount sites so it updates in first-person AND on every character model the client renders. Persists in a debug-only `localStorage` key (survives respawn / round flip / reload). Defaults UNTOUCHED — a normal launch (no flag) never builds the UI, never reads storage, never installs an override.
- **HOW THE OVERRIDE REACHES BOTH SITES.** `scene.setItemTuner(store)` (called by debug.js on load + every nudge) normalizes via the shared core and re-applies to the current viewmodel + every live hunter model; `_buildHunterModel` / `setViewModel` record each held mesh's shipped-default transform (`tunerBase`) and re-apply the override at build, so respawns/rebuilds keep the tuning. Third-person position offset is authored in CHARACTER space (x=right/y=up/z=forward) and converted to bone-local via the BUILD-TIME wrist frame (same frame `heldItemBoneOffset` bakes into); first-person maps to camera space (−Z forward).
- **Files:** NEW `shared/item-tuner.js` (pure merge/normalize/export core — single source of truth shared by debug.js + scene.js + the check), EDITED `js/scene.js` (`_itemTuner` + `setItemTuner`/`_tunerFor`/`_applyItemTunerToCtl`/`_applyItemTunerToViewModel`; base-transform + grip-frame capture at both mount sites; import), `js/debug.js` (the tuner UI section, gated on `ctx.debugFlag`; localStorage; steppers/reset/export; scene re-push on identity change; imports the shared core — its ONLY import), NEW `tools/check-item-tuner.mjs` (exercises the real merge/export logic + asserts both mount sites read the override + defaults untouched), EDITED `tools/check-debug-menu.mjs` (invariant relaxed: debug.js may import ONLY the pure item-tuner core), `notes/held-item-tuner.md`, `notes/debug-menu.md`, `architecture.md`, this file.
- **VERIFY:** `node tools/check-item-tuner.mjs` ALL GREEN (46 ✓ — merge layers over defaults, per-item independence, export round-trips to valid config, both mount sites wired, no default change). `check-debug-menu` / `check-blindfold` / `check-tool-visibility` / `check-hunter-model` / `check-held-item-offset` / `check-flicker` GREEN. `browser_check` boots clean (0 console errors) on `?debug=1` AND a normal launch — the relative shared-core import resolves at runtime. **OWED — VRmike's eyeball (the human-in-the-loop premise, NOT a gap):** headless can't render, so the ACTUAL alignment is VRmike's to tune. Open `?debug=1`, tune each item until the grip looks right in first-person AND on the model (solo via free cam / a friend watching), Export, paste in chat → a follow-up build bakes it into `shared/config`.

## Latest: VOTE-KICK CLICK REGRESSION — dead "vote kick" button fixed (#213, 2026-07-20, VRmike, branch build/213-vote-kick-click-regression). After #210 the scoreboard "vote kick" button flashed but never activated for ANYONE — no vote, no banner. DIAGNOSIS-LED (not a blind patch): the whole INITIATE path (button→`onVoteKick`→`C2S.START_VOTEKICK`→referee `startVoteKick`→snapshot banner) is correct and was unchanged by #210; the referee + check-votekick (which drives the referee DIRECTLY) both passed, which is why #210 shipped the dead button. The fault was purely client DOM. FULL DETAIL: `notes/vote-kick.md` (Regression section).

- **ROOT CAUSE.** `ui.updatePauseScoreboard` did `list.innerHTML = ''` + full recreate of every row (INCLUDING the vote-kick `<button>`) on **every ~15 Hz snapshot** while the pause menu is open (js/main.js:1165 live refresh; the host hits it too via the loopback in net.js:276). A deliberate click outlasts the ~66 ms between snapshots, so the button was torn out of the DOM between mousedown and mouseup → the browser fires `click` on the surviving common ancestor, never on the button → `onVoteKick` never ran. #210 didn't add the rebuild, but by (correctly) making the button appear for EVERY player it turned a mostly-unnoticed flaky click into an everyone-dead one.
- **THE FIX (`js/ui.js updatePauseScoreboard`).** Reconcile rows **in place keyed by player id** (`this._psRows` map) instead of nuking + recreating. Row + button NODES are reused across refreshes; only mutable bits (name/role, health, `disabled`/`title`, ordering via `insertBefore` which MOVES not destroys) are patched. The action cell is rebuilt only when its TYPE changes (button ↔ host-note ↔ none); the click handler is attached ONCE at creation (safe — a row is keyed to one id). `innerHTML` clears only in the empty-roster branch. All #210 behaviour preserved (disguise-leak rule, host "can't kick" note, vote-active/cooldown greying, live health). No referee/protocol/netcode change.
- **Files:** EDITED `js/ui.js` (`updatePauseScoreboard` in-place reconcile), `tools/check-votekick.mjs` (NEW **K** initiate-from-`START_VOTEKICK`-message via the real `ref.handleMessage` router — the exact path that regressed, asserts vote opens + banner rides the next snapshot to every player incl. initiator, and casts-via-router still resolve; NEW **L** static guard that the scoreboard reconciles in place — `_psRows`/`insertBefore`/no per-refresh `innerHTML` wipe), `notes/vote-kick.md`, this file.
- **VERIFY:** `node tools/check-votekick.mjs` ALL GREEN (A–L, incl. the new initiate-from-click coverage the old suite lacked — that gap is why #210 passed with a dead button). Neighbour sweep GREEN: `check-blindfold`, `check-round-flip-blindfold`, `check-lifecycle`. `browser_check` boots clean (0 console errors, desktop; menu renders). **OWED — VRmike 2-client in-game:** host + a peer, click "vote kick" on another row → banner pops for BOTH, votes tally, changing a pick still highlights (#210's feature), and the button no longer just flashes. Headless can't drive a 2-peer DOM vote, so the live click confirmation is on the humans.

## Latest: RIFLE DAMAGE — SMALLEST-PROP FRAGILITY BOUNDARY (#204, 2026-07-20, VRmike, branch build/204-rifle-damage-multiplier-smallest). The mustard bottle (smallest prop) took ~9%/hit (~11 rifle hits to kill) — far too tanky. Now the smallest props take 55%/hit and die in 2 rifle hits. Medium/large props UNCHANGED. Host-side damage math only. FULL DETAIL: `notes/balance-tuning.md` (SMALLEST-PROP FRAGILITY section) + `notes/hunter-tools-combat.md`.

- **THE FIX (`shared/damage.js sizeMultiplier`).** Added a small-size BOUNDARY: props whose footprint size is `<= damage.smallPropSize` (0.68 m) take the `smallMult` multiplier FLAT instead of the hyperbola. `base 5 × smallMult 11 = 55%` health/hit ⇒ dead in 2 hits. Everything ABOVE 0.68 m stays on the exact same `pivot/size` curve — medium/large did not move.
- **WHY the premise was half-right.** The task guessed the formula was "clamping/compressing" tiny props. It was NOT clamping (smallMult ceiling 10 never bound — the smallest prop only reached raw mult 2.27); the hyperbola just gives shallow ~1.85× values near the pivot for the 0.5–0.65 m cluster. So this is a genuine ~6× buff to the smallest props, NOT an un-clamp. Reaching 55% also required raising `smallMult` 10→11 (old ceiling capped at 50%).
- **THE KNOBS (rules.json `damage`, hot-tunable).** NEW `smallPropSize` 0.68 (the "smallest prop" size cutoff — sits in the real size GAP between the tiny-food cluster ≤0.647 m and the medium burger/plate/cheese cluster 0.713 m, so it's robust to hull-rebake jitter). `smallMult` 10→**11** (now the ceiling AND the flat smallest-prop multiplier). Raise/lower `smallPropSize` to widen/narrow which props count as "smallest".
- **SCOPE CAVEAT (flagged — VRmike didn't answer a clarifying question, defaults chosen).** "Smallest prop" is a CLUSTER, not just mustard: potato (0.477) & tomato (0.487) are actually SMALLER than mustard (0.584), plus kitchen_stool, ketchup, stew_bowl, lettuce, onion all sit ≤0.647 m. ALL of them now get 55%/hit. Boundary 0.68 catches the whole tiny cluster and cleanly excludes the medium burger/plate/cheese (0.713+). If VRmike wants ONLY the mustard/ketchup bottles, lower `smallPropSize` (but ≤0.6 is fragile — mustard 0.584 vs lettuce 0.605 is a 0.02 m gap).
- **SHARED WITH GRENADE (flagged).** `sizeMultiplier` is the ONE size curve for both rifle AND grenade blast (deliberate, "one size source of truth"). So tiny props are now more fragile to grenades too: a mid-falloff blast that used to leave a mustard alive now kills it. Intended-consistent (small = fragile everywhere) but NOT asked for — surface to VRmike; scope to rifle-only would require splitting the curve (rejected as a bigger change).
- **Files:** EDITED `shared/damage.js` (`smallPropSize` in `resolveDamageCfg` + boundary branch in `sizeMultiplier`), `shared/config/rules.json` (`damage.smallPropSize` 0.68, `smallMult` 10→11, `_damageComment` extended), NEW `tools/check-smallest-prop.mjs` (permanent guard), NEW `tools/_repro_smallest.mjs` (dev balance-table dump), `notes/balance-tuning.md`, `notes/hunter-tools-combat.md`, this file.
- **VERIFY:** `node tools/check-smallest-prop.mjs` ALL GREEN (mustard 100→45→dead in 2 hits via real `referee._applyShotDamage`; burger/plate/cheese/crate/fridge/kitchen_table multipliers byte-identical to the pre-change curve; every prop ≤0.68 m flat at smallMult, every prop above on the hyperbola; boundary in a clear gap; monotonic across the seam). `node tools/check-combat.mjs` ALL GREEN (sections A–G, smallMult now 11). `browser_check` boots clean (0 console errors, desktop). **OWED — VRmike in-game:** 2 rifle shots pop the mustard bottle; confirm the wider tiny-food cluster + grenade fragility feel right (nudge `smallPropSize`/`smallMult` if not).

## Latest: HUNTER HELD-ITEM OFFSET — REDO, real root cause (#203, 2026-07-20, VRmike, branch build/203-hunter-held-item-offset). Third pass on "rifle/grenade/finder float off the hunter's hand on remote models" — #188 (forward) and #190 (down) never showed in-game. ROOT CAUSE: the offset was computed in the BIND (A-)pose but the item renders in the AIM pose. Config numbers were already right; the FRAME they were applied in was wrong. FULL DETAIL: `notes/hunter-character-model.md` (2026-07-20 HELD-ITEM OFFSET REDO head section).

- **THE BUG.** `_buildHunterModel` baked the held item's offset as a fixed bone-local vector ABOVE where the AnimationMixer is created — i.e. from the wrist bone's BIND (A-)pose orientation. The item RENDERS while the mixer poses the rig in the aim clips (`Idle_Gun_Pointing`/`Run_Shoot`), where the wrist is rotated ~90°. So a "down+forward" bind-pose vector maps to up/sideways once the arm raises. Proven headless (`tools/_probe_posed_offset.mjs`): shipped config (fwd 0.22, down 0.17) rendered in the aim pose as forward +0.24 m but down **−0.14 m (UP)** — the #190 down nudge was actively LIFTING the item. This is the sizing-saga lesson again: the old check asserted the offset CODE existed, not its OUTPUT.
- **THE FIX — pose first, then anchor.** `_buildHunterModel` now builds the mixer + actions up-front, plays the idle aim clip + `mixer.update(0.2)` to pose the rig into its rendered frame, and ONLY THEN computes the offset. Offset math extracted to a shared pure helper `shared/hunter-sizing.js::heldItemBoneOffset` (browser + check run the SAME code). CONFIG UNCHANGED (forwardOffset 0.22, downOffset 0.17 — already in the requested 0.15–0.20 m band). `_boneWorldScale` folded into the helper + removed; `_boneLocalDir` kept (look-pitch uses it).
- **Files:** EDITED `js/scene.js` (pose-first reorder in `_buildHunterModel`, offset via shared helper, removed `_boneWorldScale`, import), `shared/hunter-sizing.js` (NEW `heldItemBoneOffset`), `shared/config/character-models.json` (`_forwardOffsetComment` rewritten — numbers were right, frame was wrong; DON'T re-inflate magnitude to compensate), `tools/check-hunter-model.mjs` (§5 updated: shared helper + pose-first source-order check), NEW `tools/check-held-item-offset.mjs` (OUTPUT gating check), NEW `tools/_probe_posed_offset.mjs` (diagnosis), `notes/hunter-character-model.md`, this file.
- **VERIFY:** `node tools/check-held-item-offset.mjs` ALL GREEN — loads real GLB, poses into `Idle_Gun_Pointing`, runs shipped `sizeHunterRig`+`heldItemBoneOffset`: grip lands forward 0.220 m, down 0.170 m (in the 0.15–0.20 band), on the hand side (forearm-dir dot 0.77), hand-scale displacement, STABLE across yaw 0/115/−80° (the case a bind-pose bug fails). `check-hunter-model` / `check-hunter-model-size` / `check-tool-visibility` / `check-blindfold` / `check-flicker` GREEN. `browser_check` boots clean (0 console errors, desktop). **OWED — live 2-client screenshot:** headless can't render a remote hunter; #190 claimed victory without one, so VRmike must eyeball the grip in-hand across rifle/grenade/finder. Hot-tunable (forwardOffset/downOffset) if it needs a nudge — but check the pose-frame first, don't just inflate the number (that's what created the loop).

## Latest: GRENADE BLAST RADIUS — NEAREST SURFACE DISTANCE (#202, 2026-07-20, VRmike, branch build/202-grenade-blast-radius-use). Grenades measured distance to an object's CENTRE, so big prop players (fridge/table) shrugged off blasts visibly touching their side — you had to hit the pivot. Now measured to the nearest point on the SURFACE. Host-side referee + one physics query; radius/falloff curve UNCHANGED. Side effect (intended, VRmike): big props take MORE grenade damage/fling (surface is closer ⇒ falloff scales up); if fridges feel too fragile, that's a follow-up balance knob, not a bug. FULL DETAIL: `notes/hunter-grenades.md` (NEAREST-SURFACE DISTANCE section).

- **THE PREFERENCE ORDER (`referee._blastDist(center, physDist, pos, entry)`), fed into all three blast loops (prop-PLAYER damage, decoy BACKFIRE, FLING magnitude):** (1) LIVE Rapier collider — exact closest point via `world.projectPoint(center, solid=true, …predicate)` filtered to that ONE collider's handle (`solid=true` ⇒ a `center` INSIDE the shell ⇒ distance **0** = full damage, no negative/÷0); (2) BOUNDING-BOX fallback (`damage.boxBlastDistance`, base-seated, `halfExtentsFor`) when no live collider — offline guard or edge states, never worse than centre; (3) plain CENTRE (`dist3`) ONLY for an unknown-size target (undisguised player). Clamped `>= 0`. NEVER a silent centre fallback for a sized target.
- **PHYSICS (`shared/physics.js`):** new `nearestPropSurfaceDistance(propId, center)` (dynamic body OR capped-static — both in `_propHandleToId`) + `nearestPlayerSurfaceDistance(playerId, center)` (a disguised player's MOVEMENT collider = the true prop shape from `setPlayerCollider`, so a fridge-player measures from the fridge side exactly like a real fridge), both via a shared `_nearestSurfaceDistance` projectPoint helper; return `null` when the collider isn't queryable.
- **FLING direction unchanged:** magnitude now scales off surface distance, but the shove DIRECTION is still derived inside `applyBlastImpulse` (body pos vs `center`) — measuring point and push direction decided separately, so big props fly the right way.
- **Files:** EDITED `shared/physics.js` (surface-distance methods), `shared/referee.js` (`_blastDist` + all three loops), `shared/damage.js` (`boxBlastDistance`), `tools/check-grenade.mjs` (§K new + §D/§I updated to surface-based, relationship-derived), NEW `tools/check-grenade-surface.mjs` (real-Rapier live guard, SKIP+exit-3 if WASM absent, exits via `process.exitCode`), `notes/hunter-grenades.md`, this file. This build RESUMED an interrupted attempt-2 whose physics/referee/damage edits were already in the working tree; this session verified them, fixed the two stale center-based guard assertions (§D/§I), and added the §K + real-Rapier guards.
- **VERIFY:** `node tools/check-grenade.mjs` ALL GREEN incl. new §K (table player whose PIVOT is beyond `outer` but whose SIDE the blast touches now takes real damage; near-surface > far; live-collider preferred over box/centre; dist-0 & defensive-negative ⇒ full; physics-null ⇒ box; undisguised ⇒ centre). `node tools/check-grenade-surface.mjs` GREEN (real Rapier: a 2.4 m table's surface distance 3 m off its pivot ≈ 1.8 m = centre − half-width, 0 inside, monotonic, null for unknown id; table-disguised player measured from the box side). `check-object-sync` / `check-solid-players` / `check-prop-mass` GREEN; `browser_check` boots clean (0 console errors, desktop). **OWED — live 2-client feel pass:** throw a grenade at the SIDE of a fridge/table-disguised player (pivot metres away) → real damage/fling now; confirm it's not *too* punishing (if so, nudge `grenade` radii / `baseDamage` in rules.json).
...
## Latest: VOTE-KICK VISIBILITY + PARTICIPATION FIX (#205, 2026-07-20, VRmike, branch build/205-vote-kick-visibility-participation). Playtest follow-up to #194. FULL DETAIL: `notes/vote-kick.md`.
- **INITIATOR NOW PARTICIPATES.** The initiator was latched as an un-changeable auto-YES → their banner showed but with NO buttons (read as "no banner / can't take part"). Fix: the initiator's auto-YES is now a DEFAULT, not a lock. `referee.castVote` accepts a CHANGE (re-cast overwrites, never double-counts, doesn't perturb early resolution). Client keeps the Yes/No buttons LIVE for every elector and highlights the current pick (`state.myVoteChoice` → `ui.setVoteKick(vote, selfId, myChoice)`, `.chosen` CSS). Initiator sets an optimistic `myVoteChoice=true` on click; can flip to NO and watch. Everyone (initiator included) sees the same live tally — this was already broadcast on every snapshot, so no netcode change.
- **"ONLY HOST SEES BUTTONS" was the 2-player host-exclusion, not a viewer gate.** `updatePauseScoreboard` already draws a kick button on every OTHER player's row for EVERY viewer (host + guests). The one exclusion is the host as a TARGET (kicking the server ends the match; needs host migration). In VRmike's 2-player test the guest's only other player IS the host → zero buttons. DECISION (host-kickable question went unanswered): kept the host unkickable (safe) and added a greyed, non-interactive "host · can't kick" note (`.ps-kick-host`) on the host's row so it reads as intentional, not broken. With 3+ players guests already see real buttons. If VRmike truly wants the host votable, that needs host migration — flagged, not built.
- **Files:** EDITED `shared/referee.js` (castVote allows re-cast), `js/main.js` (`state.myVoteChoice`; castVote flip; initiator optimistic YES; snapshot/reset plumbing), `js/ui.js` (setVoteKick keeps buttons live + highlights; host-row note), `css/style.css` (`.votekick-btn.chosen`, `.ps-kick-host`), `tools/check-votekick.mjs` (new **J) vote-change** section), `notes/vote-kick.md`, this file.
- **VERIFY:** `node tools/check-votekick.mjs` ALL GREEN (incl. new flip section). `browser_check` boots clean (0 console errors, desktop). **OWED — live 2-client playtest** (host initiates + guest initiates, both flip YES↔NO): couldn't drive two P2P clients headless from the sandbox.

## Superseded: VOTE-KICK SYSTEM (#194, 2026-07-19, VRmike, branch build/194-vote-kick-system-vrmike). Player-driven kicks — the HUMAN replacement for the automatic AFK-boot removed by #192 (liveness). Host-authoritative; clients only ASK. FULL DETAIL: `notes/vote-kick.md` + `architecture.md` (Vote-kick section).
