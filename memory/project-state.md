...
- **Files:** EDITED `shared/config/rules.json` (leaveTimeoutSeconds 15 + rewritten comment), `js/net.js` (`_startKeepalive`/`_markAlive`/`onKeepalive`, always-on keepalive, fail-silent `_sendPings`, removed `enablePing`/`_pingOn`), `shared/referee.js` (`markSeen()` + liveness comments + default 15), `js/main.js` (`session.onKeepalive`→watchdog feed, `hostSilenceMs` comment/default 15, dropped `if(DEBUG)enablePing`), `tools/check-lifecycle.mjs` (A′: markSeen wiring + AFK-stays-5-min + dead-connection-swept + boundary), `tools/check-host-disconnect.mjs` (D: onKeepalive/keepalive/markSeen wiring), `tools/check-debug-menu.mjs` (always-on keepalive asserts), notes + this file. NO protocol message added (the ping was already a control frame), NO physics/damage/render/referee-gameplay change.
- **VERIFY:** `node tools/check-lifecycle.mjs` ALL GREEN incl. new A′ (AFK player with pings-only survives 5 simulated minutes; ping-silence past 15s still sweeps; a peer 0.5s under the threshold is NOT swept). `check-host-disconnect` / `check-debug-menu` / `check-blindfold` / `check-spectator` GREEN. `browser_check` boots clean (0 console errors). **OWED — live 2-window pass:** background the guest tab for a few minutes with NO input (bathroom sim) → stays in the round; then hard-close the guest tab → host sweeps it within ~15s and the guest side (if host dies) boots to lobby with "Lost connection to host."
